...
def write_city(cfg: RegimeConfig, cities_dir: Path | None = None) -> Path:
    city_dir = (cities_dir or CITIES_DIR) / cfg.name
    ...
    shown = city_dir.relative_to(ROOT) if city_dir.is_relative_to(ROOT) else city_dir
    print(f"wrote {cfg.name}: {n_segments} segments, {n_reports} reports -> {shown}")
    return city_dir
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        help="write the cities under this directory instead of benchmarks/cities. "
        "The generated bytes do not depend on where they land (the city config names "
        "its inputs relatively), so this is what lets a gate regenerate the suite "
        "WITHOUT overwriting the committed copy it is about to compare against.",
    )
    args = parser.parse_args()
    names = args.config or sorted(p.stem for p in CONFIGS_DIR.glob("*.json"))
    for name in names:
        cfg = RegimeConfig.from_json(CONFIGS_DIR / f"{name}.json")
        write_city(cfg, args.out)
