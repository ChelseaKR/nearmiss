...
The grid cells are indexed by their (x, y) coordinates in the projected plane.
An item at (x, y) falls in cell (x // cell_size, y // cell_size).
...
class SpatialIndex:
    ...
    def neighbors_in_radius(
        ...
        vertex of a polyline shares that polyline's id. At most one entry per id
        is returned, but *any* one of its instances passing the distance filter
        is enough: an id is only excluded from further consideration once one of
        its instances has actually been included, never merely because some
        other, farther instance of the same id was the first one visited. (An
        id's instances can straddle the bounding square too — some inside
        radius_m, some outside — so visit order must not let an out-of-range
        instance shadow an in-range one.)
        """
