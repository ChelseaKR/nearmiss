...
- **Pinned and hashed dependencies.** Committed and CI-enforced. `requirements.lock` (`make lock`,
  ...
  pytest, ruff, mypy, pip-audit, babel) are both generated by `pip-compile --generate-hashes`. Every
  CI job installs from `requirements-dev.lock` with `pip install --require-hashes`, then the local
  package `--no-deps -e .` — not a fresh, unpinned `pip install -e ".[dev]"` resolve. Dependabot
  regenerates both lockfiles' pins and hashes on a dependency bump.
- **`pip-audit --strict`** runs in CI on every push/PR, blocking (no `|| true`); a new advisory against
  a pinned dependency fails the merge gate, and is treated as a security event, not a routine bump.
- **`gitleaks`** runs in CI on every push/PR to catch secrets before they reach history, plus a
  separate scheduled full-history scan (`secret-scan-scheduled.yml`).
- **CodeQL** runs in CI (Python, JavaScript, and the GitHub Actions workflow YAML itself) with the
  `security-extended` query pack.
- **`zizmor`** runs in CI as a merge-blocking GitHub Actions workflow SAST gate (`min-severity: high`).
- **GitHub Actions are SHA-pinned**, not tag-pinned (every `uses:` carries a full 40-character commit
  SHA with a `# vN` comment); Dependabot and Renovate both keep the pinned digests current.
- **Dependabot** is configured (`.github/dependabot.yml`) and opens dependency-update and
  security-update PRs, which go through the same lint, type, test, and audit gates as any change.
