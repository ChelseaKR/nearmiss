...
| **SBOM generation** | **Exercised.** The tag-triggered release workflow builds a CycloneDX 1.7 SBOM and attaches it to the release. `sbom.cdx.json` is published on the v0.2.0, v0.3.0, and v0.3.1 releases (2026-08-08), each with a Sigstore bundle, so there is a published SBOM to verify. |
| **Artifact signing** | **Exercised.** Keyless cosign signing and SLSA provenance ran on all three tags; the releases carry `attestation.json` plus Sigstore bundles for the sdist, wheel, SBOM, and every published city GeoJSON. The 0.3.1 wheel's attestation resolves through the GitHub attestations API. |
