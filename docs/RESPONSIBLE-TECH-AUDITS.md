...
| **SBOM generation** | **Not yet implemented.** No release pipeline exists to attach one (see P1-1 in the remediation plan — a tag-triggered release workflow with CycloneDX SBOM generation is planned, not built). Declared here as a gap, not silently omitted. |
| **Artifact signing** | **Not yet implemented — and no version has ever been signed**, despite `CHANGELOG.md` previously claiming otherwise before this pass's correction (see `CHANGELOG.md`'s 2026-07-05 note). Keyless cosign signing is planned as part of the same future release pipeline (P1-1). |
