...
### FIX-01 — Land the orphaned research branch onto diverged main — ✅ DONE (2026-07-02)
- **Status:** Landed. `research-panel-and-roadmap` (`6dfabb0`) merged across the 8
  diverged `main` commits: `stats/maup.py` (RR-05) and `pearson_dispersion`/`quasi_poisson_ci`
  in `stats/rates.py` (RR-02) are on `main` with `RESEARCH-ROADMAP.md`/`USER-RESEARCH.md`
  and `tests/test_maup.py`/`test_overdispersion.py`. The i18n conflict (branch's pre-gettext
  `_EN` dict vs. the `#25` gettext migration) was resolved on main's side and the branch's
  new brief strings were re-extracted into `locales/messages.pot` + en/es `.po`/`.mo`
  (`make i18n` green). `msgpack>=1.2.1` preserved. Wiring done: `publish.py` emits
  `methods.dispersion.phi` and a `maup_rank_stability` block (verified for both demo cities);
  `brief.py` renders a robustness section (EN + ES). METHODOLOGY §4/§8 and LIMITATIONS §5
  now read "implemented, config-gated"; the `[tool.mutmut]` note no longer denies `maup.py`
  exists and now scopes it. `make verify` passes on the merged result.
...
### FIX-05 — First-class analysis window
