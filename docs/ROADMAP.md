...
| City-scale performance (work units) | Every benchmark work-unit metric within 10% of the committed baseline, direction-aware | `make perf-budget`; [`perf/baseline.json`](../perf/baseline.json), [`perf/README.md`](../perf/README.md) | AUTO | Maintainer |
| City-scale performance (wall clock) | Recheck the committed 300/6,000 and 800/20,000 timings; investigate a >10% move. Not automatable: shared CI runners vary by more than the band, so the AUTO row above budgets exactly reproducible work units instead and cannot see a constant-factor slowdown | `make bench`; [`PERFORMANCE.md`](PERFORMANCE.md) | REVIEW | Maintainer |
