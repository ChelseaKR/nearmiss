...
Adding a language touches **`locales/` only** — no JS or HTML change:

1. `pybabel init -i src/nearmiss/locales/messages.pot -d src/nearmiss/locales -l <lang>`,
   then translate every `msgstr` (brief **and** `web.*`).
2. `make i18n-compile` — builds the new `.mo` **and** `web/locales/<lang>.json`.
3. Add the tag to `SUPPORTED_LANGUAGES` in `i18n.py`; `make i18n`.
