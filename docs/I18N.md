...
| **G5-U** | Untranslated source (no verbatim English in `es`) | same script: a non-`en` translation may not be byte-identical to its English source **when the source carries translatable words**. See [What this gate can and cannot see](#what-the-untranslated-source-gate-can-and-cannot-see). |
...
**The escape hatch is `IDENTICAL_OK`** in `tools/check_catalog_parity.py`: a
`{locale: {msgid: reason}}` map for a translation that is deliberately identical.
It is empty today. Adding a line there is how a genuine false positive is
resolved — which keeps the set of identical strings an auditable register rather
than an invisible hole, and keeps the answer to a false positive from being
"disable the gate".
