...
| **603.2 Information on Accessibility and Compatibility Features (support services)** | Supports | Support is provided through the GitHub repository (private during development). Accessibility issues can be filed via issue templates, and the maintainer treats accessibility regressions as merge-blocking — the structural gate is part of `make verify` and the axe-core run is wired via `make axe` and CI; this ACR names the contact and the standards targeted. |
