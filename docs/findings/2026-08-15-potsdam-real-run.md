...
- Surface the singleton-neighbourhood case in the output instead of silently emitting a z.
  ...
  in `quality_flags` so a ★ in the brief always means a cluster. This is filed separately.
- Reconsider the `gi_band_m`-versus-segment-length interaction. A band that cannot span the
  units it is applied to is a configuration error the tool should detect and say so, since the
  affected segments are exactly the long arterials most likely to carry reports.
