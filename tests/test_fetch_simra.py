...
This lands the previously-orphaned `origin/claude/real-data-map-integration-mud09x`
branch's fetch_simra tests onto the SourceAdapter framework (EXP-04): the module
under test is now `nearmiss.adapters.simra` via the `tools/fetch_simra.py` CLI
shim, and the hazard crosswalk comes from `crosswalks/simra.toml`.
"""
