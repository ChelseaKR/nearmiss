...
reproduce: ## HR5: rebuild the published dataset + brief from raw inputs; fail if output changed
	$(PYTHON) -m $(PACKAGE) run --config $(CONFIG) --out build/brief.md
	git diff --exit-code -- $(PUBLISHED_DIR)
	@echo "reproduce: published dataset regenerated byte-for-byte from raw inputs."
