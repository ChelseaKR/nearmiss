...
| Security & Supply-Chain | Applies | `pip-audit --strict` (blocking, no mute) + gitleaks + locally enforced CodeQL run in CI, from a hashed dev-toolchain lock (`requirements-dev.lock`, `--require-hashes`); the SBOM/signing/SLSA pipeline (`.github/workflows/release.yml`) has been **exercised on v0.2.0, v0.3.0, v0.3.1, and v0.4.0** (2026-08-08 and 2026-08-16): each cut a GitHub Release carrying a CycloneDX SBOM, Sigstore bundles for the sdist, wheel, SBOM, and every published city GeoJSON, and a SLSA provenance attestation. ASVS 5.0 L2 is declared; the control-by-control checklist remains open (see [Security](#security) and [`docs/RESPONSIBLE-TECH-AUDITS.md`](docs/RESPONSIBLE-TECH-AUDITS.md)). |
| CI/CD | Applies | The core pipeline (lint/type/test/i18n/accessibility/security/reproducibility) is merge-blocking; high-severity `zizmor` and CodeQL for GitHub Actions and web JS are enabled. The live ruleset is committed at [`.github/rulesets/main.json`](.github/rulesets/main.json) with its admin bypass stated rather than omitted, and `tests/test_ruleset.py` holds it to the workflows. Capturing it surfaced that **four jobs run on a pull request and cannot block it**, including `claims`, the docs-code drift gate; they are enumerated with a reason each so a fifth cannot appear silently. |
| Release & Versioning | Applies | SemVer + [`CHANGELOG.md`](CHANGELOG.md) are maintained; the release pipeline (`.github/workflows/release.yml`) has run for real. **v0.2.0, v0.3.0, v0.3.1, and v0.4.0 are annotated, SSH-signature-verified tags** (2026-08-08 and 2026-08-16), each with a signed GitHub Release. **PyPI Trusted Publishing is registered and working** — `nearmiss-safety` 0.3.0, 0.3.1, and 0.4.0 are on PyPI (the name `nearmiss` was already taken; see `aea68e3`). The one thing still outstanding is a DOI (`CITATION.cff`), tracked by [#227](https://github.com/ChelseaKR/nearmiss/issues/227), which the marker at `CITATION.cff:68` points at. It previously read `TODO(#184)` and satisfied CQ-34's no-bare-marker gate while [#184](https://github.com/ChelseaKR/nearmiss/issues/184) was closed and about stale tag claims in this file rather than about minting a DOI — see [`docs/ROADMAP.md`](docs/ROADMAP.md) § Open review and owner actions for what that gate still cannot see. |
...
| Internationalization | Applies | <!-- claim:i18n-gate-count -->The strongest area: gettext catalogs with six merge-blocking gates in `make i18n` (POT drift, `msgfmt --check`, EN/ES parity, web-JSON drift, BCP-47 validity, and the pseudo-locale no-bypass gate) — see [`docs/I18N.md`](docs/I18N.md).<!-- /claim:i18n-gate-count --> |
| AI Development Measurement | Applies | Delivery and quality signals are recorded in [`docs/ROADMAP.md`](docs/ROADMAP.md); AI-specific diagnostic baselining introduced by v2.0.0 remains an open review item. |
| AI Evaluation | N/A — no LLM/model component | No LLM/AI SDK usage anywhere in the codebase (verified: a grep for `anthropic`/`openai`/`langchain`/`bedrock`/generic LLM-client imports across `src/` and `tools/` is clean). See [`docs/adr/0004-standards-applicability.md`](docs/adr/0004-standards-applicability.md) — this flips to **Applies** immediately, per the standard's own AIEV-01, the moment any LLM-backed feature is added (e.g. an AI-assisted moderation triage or an auto-summarized brief). |
| Data Governance | Applies | Source lineage, licensing, privacy tiers, and retention are documented in [`docs/DATA-CARD.md`](docs/DATA-CARD.md), with retention enforcement covered by contributor-data tests. |
...
| Incident Response | Applies | Security reporting and operational integrity guidance exist in [`SECURITY.md`](SECURITY.md) and [`docs/LIVE-INTEGRITY.md`](docs/LIVE-INTEGRITY.md); the v2.0.0 incident-label and postmortem convention remains to be adopted. |
| Performance | Applies | <!-- claim:perf-regression-budget -->The city-scale benchmark and current baseline are documented in [`docs/PERFORMANCE.md`](docs/PERFORMANCE.md) and run through `make bench`, and the regression budget is now merge-blocking: `make perf-budget` (in `make verify` and in CI's required `reproducibility` job) re-measures the benchmark and fails on any metric more than 10% off the committed [`perf/baseline.json`](perf/baseline.json), per [`docs/standards/PERFORMANCE-STANDARD.md`](docs/standards/PERFORMANCE-STANDARD.md) §2. The budgeted metrics are exactly reproducible work units rather than wall-clock seconds, because a seconds budget on a shared CI runner can be merge-blocking or honest and not both; what that leaves uncovered — a constant-factor slowdown inside an unchanged number of calls — is stated in [`perf/README.md`](perf/README.md) and stays a REVIEW row in [`docs/ROADMAP.md`](docs/ROADMAP.md).<!-- /claim:perf-regression-budget --> |
| Responsible-Tech Framework | Applies | The threat model, initial DPIA, ASVS L2 declaration, residual-risk register, and mechanical misuse-resistance tests (k-anonymity floor, privacy leak tests, reproducibility tripwire) are committed and reviewed. The DPIA's retention-policy gap and the ASVS control-by-control checklist remain open — see [Data, privacy, and ethics](#data-privacy-and-ethics) and [`docs/RESPONSIBLE-TECH-AUDITS.md`](docs/RESPONSIBLE-TECH-AUDITS.md). |
...
# Activate one exact reviewed annual FARS accident/person lineage in private storage.
# A new 2024 lineage starts at retained revision 1, then advances to corrected revision 2.
...
nearmiss ingest-fars-year /private/downloads/FARS2024NationalCSV.zip \
  --root "$HOME/.local/share/nearmiss/ingestion" --year 2024 --contract-revision 2
...
city's data portal. <!-- claim:vuln-management-hashed-locks -->**Vulnerability** management —
pip-audit, gitleaks, CodeQL configured in CI; dependencies install from the committed hashed locks
(`requirements.lock` and `requirements-dev.lock`, both generated by `pip-compile --generate-hashes`)
with `--require-hashes` — see [Install](#install).<!-- /claim:vuln-management-hashed-locks -->
**Auditability** — the audit policy and naming convention are committed in
...
a short track record (four tags to date, v0.4.0 current).
...
<!-- claim:standards-table-cross-reference -->the [standards table](#standards-conformance) for how
this fits the other fourteen standards.<!-- /claim:standards-table-cross-reference -->
...
analysis runs; conventional commits; pinned, SLSA-friendly GitHub Actions; a dispatch-gated SBOM/Sigstore/SLSA release pipeline,
exercised on four tags — see `.github/workflows/release.yml` and "How to verify a release" in
...
<!-- claim:supported-versions-not-restated -->
**Supported versions:** the latest released minor line (best-effort backport of high/critical fixes
to the previous minor). Which line that is follows the newest release tag rather than a number typed
into this sentence, because a number typed here is what went stale last time — see the full table in
